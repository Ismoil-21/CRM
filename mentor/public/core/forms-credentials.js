...
function updateTopbar(tab){
  ...
  const titles={
    ...
    'coin-shop':         L==='ru'?'🪙 Монетный магазин':L==='en'?'🪙 Coin Shop':'🪙 Coin Shop',
    'student-coin-shop': L==='ru'?'🛍️ Монетный магазин':L==='en'?'🛍️ Coin Shop':'🛍️ Coin Shop',
  };
  const subs={
  ...
  if(tab==='coin-shop'){document.getElementById('tb-sub').textContent=L==='ru'?'Магазин · Баланс менторов · Покупки':L==='en'?'Shop · Mentor balances · Purchases':'Dokon · Mentor balanslari · Xaridlar';}
...
function go(tab,el){
  ...
  if(tab==='settings'){setTimeout(()=>{renderSettingsPanel();if(typeof isMentorRole==='function'&&isMentorRole()){['nav-dashboard','nav-courses','nav-groups','nav-mentors','nav-students','nav-finance'].forEach(function(id){var el=document.getElementById(id);if(el)el.style.display='none';});}if(typeof isStudentRole==='function'&&isStudentRole()){['nav-dashboard','nav-courses','nav-groups','nav-mentors','nav-students','nav-finance','nav-mentor-dash','nav-mentors-my','nav-mentor-chat','nav-tests-mentor','nav-grades-mentor','nav-tests','nav-grades'].forEach(function(id){var el=document.getElementById(id);if(el)el.style.display='none';});}},50);}
  ...
  if(tab==='coin-shop'&&typeof renderAdminCoinShop==='function')renderAdminCoinShop();
  if(tab==='student-coin-shop'&&typeof renderStudentCoinShop==='function')renderStudentCoinShop();
...
function initApp(){
  Object.assign(D,loadData());loadUI();
...
function saveStudentCredentials(){
  ...
  const conflict=existing.find(u=>u.login===login&&String(u.studentId)!==String(sel.value));
  if(conflict){toast('⚠️ Bu login allaqachon band!');return;}
  // Mentor loginlari bilan to'qnashmasin (sync check)
  const mentorConflict=getMentorUsers().find(u=>u.login===login);
  if(mentorConflict){toast('⚠️ Bu login mentor uchun band!');return;}
...
(function(){
  ...
  if(hint) hint.textContent='🔐 Admin: admin / admin123';
})();
...
window.__crmBoot = function(){
  ...
  } catch(e){ console.error('[CRM Boot]', e); }
};
