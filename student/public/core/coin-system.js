...
(function () {
        ...
        .map(function (item) {
          ...
          return (
            ...
            (item.image
              ...
              : '<div style="height:100%;display:flex;align-items:center;justify-content:center;font-size:52px">🎁</div>') +
            (so
              ? '<div style="position:absolute;inset:0;background:rgba(0,0,0,.52);display:flex;align-items:center;justify-content:center"><span style="color:#fff;font-size:14px;font-weight:800;border:2px solid rgba(255,255,255,.7);padding:4px 14px;border-radius:20px">' +
            ...
            escJs(item.id) +
            '\')" style="flex:1;padding:8px;border-radius:10px;border:1.5px solid var(--border2);background:var(--bg2);font-size:12px;font-weight:700;cursor:pointer" onmouseover="this.style.background=\'#eff6ff\'" onmouseout="this.style.background=\'#f8fafc\'">✏️ ' +
            tl("Tahrir", "Изменить", "Edit") +
            "</button>" +
            "<button onclick=\"csDeleteProduct('" +
            escJs(item.id) +
            '\')" style="padding:8px 12px;border-radius:10px;border:1.5px solid #fecaca;background:var(--bg)5f5;color:#ef4444;font-size:12px;font-weight:700;cursor:pointer" onmouseover="this.style.background=\'#fee2e2\'" onmouseout="this.style.background=\'#fff5f5\'">🗑</button>' +
            "</div>" +
            "</div>" +
            "</div>"
          );
      ...
      sorted
        .map(function (p, i) {
          return (
            ...
            "</td>" +
            '<td style="text-align:center;padding:11px 14px">' +
            '<span style="padding:4px 12px;border-radius:20px;font-size:11px;font-weight:700;background:' +
            (p.approved ? "#d1fae5" : "#fef3c7") +
            ";color:" +
            (p.approved ? "#065f46" : "#92400e") +
            '">' +
            (p.approved
              ? "✅ " + tl("Tasdiqlangan", "Подтверждено", "Approved")
              : "⏳ " + tl("Kutilmoqda", "Ожидание", "Pending")) +
            "</span>" +
            (!p.approved
              ? "<br><button onclick=\"csApprove('" +
                escJs(p.id) +
                '\')" style="margin-top:4px;padding:3px 10px;border-radius:8px;border:1px solid #6ee7b7;background:var(--teal-light);color:var(--teal-text);font-size:11px;font-weight:700;cursor:pointer">' +
  ...
  window.csOpenAddProduct = function (editId) {
    ...
    box.innerHTML =
      ...
      (item && item.image
        ? '<img src="' +
          esc(item.image) +
          '" style="width:100%;height:100%;object-fit:cover">'
        : '<div style="text-align:center;color:var(--accent);pointer-events:none"><div style="font-size:40px;margin-bottom:8px">🖼</div><div style="font-size:13px;font-weight:700">' +
          tl("Rasm yuklash", "Загрузить фото", "Upload Image") +
          '</div><div style="font-size:11px;opacity:.7">' +
          tl(
    ...
    document.getElementById("csp-url").oninput = function () {
      setPreview(this.value.trim() || null);
    };
  ...
  window.csSaveProduct = function (editId) {
    ...
    if (!name) {
      alert(tl("Nomini kiriting!", "Введите название!", "Enter name!"));
      return;
    }
    if (price <= 0) {
      alert(tl("Coin narxini kiriting!", "Введите цену!", "Enter price!"));
      return;
    }
    if (isNaN(stock) || stock < 0) {
      alert(tl("Sonini kiriting!", "Введите остаток!", "Enter stock!"));
      return;
    }
    function done(img) {
      var items = getShop();
  ...
  window.csOpenSendCoin = function (pre) {
    var mentors = (window.D && window.D.mentors) || [];
    if (!mentors.length) {
      alert(tl("Mentorlar yo'q!", "Менторов нет!", "No mentors!"));
      return;
    }
    ...
    box.innerHTML =
      ...
      tl("Bekor", "Отмена", "Cancel") +
      "</button>" +
      '<button onclick="csSendCoins()" style="padding:11px 28px;border-radius:11px;border:none;background:linear-gradient(135deg,#f59e0b,#d97706);color:#fff;font-size:14px;font-weight:800;cursor:pointer">🪙 ' +
  ...
  window.csSendCoins = function () {
    ...
    if (!name) {
      alert(tl("Mentor tanlang!", "Выберите ментора!", "Select a mentor!"));
      return;
    }
    if (amt <= 0) {
      alert(
    ...
    return (
      ...
      items
        .map(function (item) {
          ...
          return (
            '<div style="background:var(--bg);border-radius:20px;overflow:hidden;border:2px solid ' +
            cardBorder +
            ";box-shadow:" +
            cardGlow +
            ';transition:.25s;position:relative" ' +
            (canBuy
              ? "onmouseover=\"this.style.transform='translateY(-6px)';this.style.boxShadow='0 16px 40px rgba(0,0,0,.16)'\" onmouseout=\"this.style.transform='';this.style.boxShadow='" +
                cardGlow +
                "';\""
              : "") +
            ">" +
            ...
            (so
              ? '<div style="position:absolute;inset:0;background:rgba(0,0,0,.58);display:flex;align-items:center;justify-content:center"><span style="color:#fff;font-size:14px;font-weight:800;border:2px solid rgba(255,255,255,.7);padding:5px 16px;border-radius:20px;letter-spacing:.05em">' +
                tl("TUGAGAN", "НЕТУ", "SOLD OUT") +
                "</span></div>"
              : "") +
            (canBuy
              ? '<div style="position:absolute;bottom:8px;left:8px;background:rgba(16,185,129,.88);backdrop-filter:blur(4px);color:#fff;font-size:10px;font-weight:800;padding:3px 10px;border-radius:20px">✅ ' +
            ...
            (so
              ...
              : canBuy
                ? "<button onclick=\"csBuyItem('" +
                  escJs(item.id) +
                  '\')" style="width:100%;padding:11px;border-radius:12px;border:none;background:linear-gradient(135deg,#10b981,#059669);color:#fff;font-size:14px;font-weight:800;cursor:pointer;box-shadow:0 4px 14px rgba(16,185,129,.38);transition:.2s" onmouseover="this.style.filter=\'brightness(1.1)\'" onmouseout="this.style.filter=\'\'">🛒 ' +
  ...
  function renderStudentPurchases(buys) {
    ...
    return (
      ...
      sorted
        .map(function (p) {
          return (
            '<div style="background:var(--bg);border:1.5px solid var(--border2);border-radius:14px;padding:14px 16px;display:flex;align-items:center;gap:14px;box-shadow:0 1px 4px rgba(0,0,0,.05)">' +
            (p.itemImage
              ? '<img src="' +
                esc(p.itemImage) +
                '" style="width:48px;height:48px;border-radius:10px;object-fit:cover;flex-shrink:0">'
              : '<div style="width:48px;height:48px;border-radius:10px;background:#fef3c7;display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0">🎁</div>') +
            '<div style="flex:1;min-width:0">' +
            ...
            "</div>" +
            coinPill(p.coinPrice || 0) +
            '<span style="padding:4px 10px;border-radius:20px;font-size:11px;font-weight:700;background:' +
            (p.approved ? "#d1fae5" : "#fef3c7") +
            ";color:" +
            (p.approved ? "#065f46" : "#92400e") +
            ';flex-shrink:0">' +
  ...
  window.csBuyItem = function (itemId) {
    ...
    if ((item.stock || 0) <= 0) {
      alert(tl("Mahsulot tugagan!", "Товар закончился!", "Out of stock!"));
      return;
    }
    ...
    box.innerHTML =
      ...
      (item.image
        ? '<img src="' +
          esc(item.image) +
          '" style="width:80px;height:80px;border-radius:14px;object-fit:cover;margin-bottom:12px;box-shadow:0 4px 16px rgba(0,0,0,.25)">'
  ...
  window.csConfirmBuy = function (itemId) {
    ...
    if (!item) {
      alert(tl("Mahsulot topilmadi!", "Не найдено!", "Not found!"));
      return;
    }
    var bal = getStudentBal(stuId);
  ...
  window.csGiveCoin = function (studentId, mentorName) {
    ...
    var stu2 = ((window.D && window.D.students) || []).find(function (s) {
      return s.id === studentId;
    });
    var groupId = stu2 ? stu2.groupId : null;
    // DOIM backend dan yangi balansni olamiz
    loadFromBackend(function () {
      var freshBal = groupId
        ? getMentorGroupBal(mentorName, groupId)
        : getMentorBal(mentorName);
      if (freshBal <= 0) {
        toast(
          tl(
            "❌ Guruh balansida coin yo'q! Admin coin bermagan.",
            "❌ Монет нет! Попросите у администратора.",
            "❌ No coins! Ask admin.",
          ),
          "#ef4444",
        );
  ...
  function _doGiveCoin2(studentId, mentorName, groupId, amt, mBal, inp) {
    if (amt > mBal) {
      toast(
        tl(
          "Coin yetarli emas! Sizda: 🪙" + mBal,
          "Монет не хватает! У вас: 🪙" + mBal,
          "Not enough! You have: 🪙" + mBal,
        ),
        "#ef4444",
      );
      return;
    ...
    if (groupId) {
      setMentorGroupBal(mentorName, groupId, mBal - amt);
    } else {
      setMentorBal(mentorName, mBal - amt);
    }
    ...
    toast(
      "✅ " +
        (stu ? stu.name : tl("Talaba", "Студент", "Student")) +
        " ga 🪙" +
        amt +
        " · " +
  ...
  window.csGiveCoinFromCell = function (studentId, mentorName, inputSuffix) {
    ...
    var stu3 = ((window.D && window.D.students) || []).find(function (s) {
      return s.id === studentId;
    });
    var groupId2 = stu3 ? stu3.groupId : null;
    // DOIM backend dan yangi balansni olamiz
    loadFromBackend(function () {
      var freshBal = groupId2
        ? getMentorGroupBal(mentorName, groupId2)
        : getMentorBal(mentorName);
      if (freshBal <= 0) {
        toast(
          tl(
        ...
        document
          .querySelectorAll('[id^="sc-bal-mentor"]')
          .forEach(function (el) {
            el.textContent = "Guruh: 0";
          });
      ...
      if (amt > freshBal) {
        toast(
          tl(
            "Coin yetarli emas! Sizda: 🪙" + freshBal,
            "Монет не хватает! У вас: 🪙" + freshBal,
            "Not enough! You have: 🪙" + freshBal,
          ),
          "#ef4444",
        );
        return;
  ...
  function _doCellGive(studentId, mentorName, groupId2, amt, mBal, inp) {
    ...
    var stuBal = getStudentBal(studentId);
    if (groupId2) {
      setMentorGroupBal(mentorName, groupId2, mBal - amt);
    } else {
      setMentorBal(mentorName, mBal - amt);
    }
    setStudentBal(studentId, stuBal + amt);
