...
// Talaba portaliga faqat Talaba kirishi mumkin
function applyStudentGuard() {
  ...
  window.showApp = function () {
    ...
    if (user.role !== 'Talaba') {
      ...
      if (errEl) { errEl.textContent = '🚫 Bu portal faqat Talaba uchun!'; errEl.style.display = 'block' }
...
onMounted(async () => {
  try {
    ...
    console.error('[Student Boot]', err)
...
.boot-error button {
  padding: .5rem 1.25rem; background: #f59e0b;
