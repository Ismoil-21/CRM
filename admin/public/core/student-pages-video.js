...
function renderStudentSchedulePage() {
  const wrap = document.getElementById("student-schedule-wrap");
  if (!wrap) return;
  ...
  if (!s) {
    wrap.innerHTML = `<div style="text-align:center;padding:60px 20px;color:var(--text3)"><div style="font-size:48px">📅</div><div style="font-size:15px;font-weight:600;margin-top:12px">${notFoundLbl}</div></div>`;
    return;
  }
  if (!grp) {
    wrap.innerHTML = `<div style="text-align:center;padding:60px 20px;color:var(--text3)"><div style="font-size:48px">👥</div><div style="font-size:15px;font-weight:600;margin-top:12px">${noGroupLbl}</div></div>`;
    return;
  }
  ...
  const DAY_SHORT =
    LANG === "ru" ? DAY_SHORT_RU : LANG === "en" ? DAY_SHORT_EN : DAY_SHORT_UZ;
  const DAY_LONG =
    LANG === "ru" ? DAY_LONG_RU : LANG === "en" ? DAY_LONG_EN : DAY_LONG_UZ;
  ...
  const nextLbl =
    LANG === "ru" ? "Следующий →" : LANG === "en" ? "Next →" : "Keyingi →";
  const daysHtml = dayOrder
    .filter((d) => d !== "Yak")
    .map((d) => {
  ...
  if (D.attendance) {
    const keys = Object.keys(D.attendance)
      .filter((k) => k.startsWith("att_" + grp.id + "_"))
      .sort();
    ...
    keys.forEach((k) => {
      ...
      if (p + a + e > 0)
        attStats.push({
          ...
          a,
          e,
          pct: Math.round((p / (p + a + e)) * 100),
          label: (monthNames[mn] || mn) + " " + yr,
        });
    });
  }
  ...
  const attHtml = attStats.length
    ? attStats
      ...
      <span style="font-size:11px;color:var(--text3)">K:${x.p} Y:${x.a} S:${x.e}</span>
        ...
        .join("")
    : `<div style="text-align:center;padding:30px;color:var(--text3);font-size:13px">${noAttLbl}</div>`;
  ...
  const schedLbl =
    LANG === "ru"
      ...
      : LANG === "en"
        ...
        : "Dars jadvali";
  const groupWord =
    LANG === "ru" ? "группа" : LANG === "en" ? "group" : "guruhi";
  const weekLbl =
    LANG === "ru"
      ? "📆 Расписание недели"
      : LANG === "en"
  ...
  const mentorLbl =
    LANG === "ru" ? "🎓 Ментор" : LANG === "en" ? "🎓 Mentor" : "🎓 Mentor";
  const mentorRoleLbl =
    LANG === "ru"
      ? "Ментор группы"
      : LANG === "en"
  ...
  const nextClassLbl =
    LANG === "ru"
      ? `⏳ Следующий урок через ${daysUntil} дн.`
      : LANG === "en"
        ? `⏳ Next class in ${daysUntil} day(s)`
        : `⏳ Keyingi dars: ${daysUntil} kundan`;
  const roomXona = LANG === "ru" ? "-каб." : LANG === "en" ? "-room" : "-xona";
  const crsLbl =
    LANG === "ru" ? "📚 Курс" : LANG === "en" ? "📚 Course" : "📚 Kurs";
  const strtLbl =
    LANG === "ru" ? "📋 Начало" : LANG === "en" ? "📋 Start" : "📋 Boshlanish";
  const durLbl =
    LANG === "ru" ? "⏱ Длит." : LANG === "en" ? "⏱ Duration" : "⏱ Davomiylik";
  const roomLabel =
    LANG === "ru" ? "🚪 Кабинет" : LANG === "en" ? "🚪 Room" : "🚪 Xona";
  const timeLabel =
    LANG === "ru" ? "⏰ Время" : LANG === "en" ? "⏰ Time" : "⏰ Vaqt";
...
function renderStudentRatingPage() {
  const wrap = document.getElementById("student-rating-wrap");
  if (!wrap) return;
  const { s, grp } = getCurrentStudentInfo();
  const noDataLbl =
    LANG === "ru"
      ...
      : LANG === "en"
        ...
        : "Ma'lumot topilmadi";
  if (!s || !grp) {
    wrap.innerHTML = `<div style="text-align:center;padding:60px;color:var(--text3)"><div style="font-size:48px">🏆</div><div style="font-size:15px;font-weight:600;margin-top:12px">${noDataLbl}</div></div>`;
    return;
  }
  ...
  const myRankLbl =
    LANG === "ru"
      ? `Ваше место: ${myRank}/${ratings.length}`
      : LANG === "en"
        ? `Your rank: ${myRank}/${ratings.length}`
        : `Sizning o'rningiz: ${myRank}/${ratings.length}`;
  const myRatingLbl =
    LANG === "ru"
      ? `Ваш рейтинг: ${myRating}%`
      : LANG === "en"
  ...
  wrap.innerHTML = `<div style="padding:0 0 32px">
    ...
    ${
      ratings.length >= 2
        ? `<div style="background:var(--bg);border:1.5px solid var(--border2);border-radius:var(--r-lg);padding:24px 22px 0;margin-bottom:20px;box-shadow:var(--shadow-sm)">
...
function renderStudentGradesPage() {
  const wrap = document.getElementById("student-grades-wrap");
  if (!wrap) return;
  const { s, grp } = getCurrentStudentInfo();
  const noDataLbl =
    LANG === "ru"
  ...
  const noCriteriaLbl =
    LANG === "ru"
      ? "Критерии оценки не добавлены"
      : LANG === "en"
        ? "No grading criteria added"
        : "Baholash mezonlari qo'shilmagan";
  const criteriaLbl =
    LANG === "ru"
  ...
  const weightLbl =
    LANG === "ru" ? "Вес" : LANG === "en" ? "Weight" : "Og'irlik";
  ...
  const totalPctLbl =
    LANG === "ru"
      ...
      : LANG === "en"
        ? "Total Percent"
        : "Umumiy foiz";
  const gradingResultsLbl =
    LANG === "ru"
      ? "Результаты оценки"
      : LANG === "en"
        ? "Grading Results"
        : "Baholash natijalari";
  const noCriteriaYetLbl =
    LANG === "ru"
      ? "Ещё нет критериев. Ментор добавит их."
      : LANG === "en"
        ? "No criteria yet. Mentor will add them."
        : "Hali mezon yo'q. Mentor qo'shganda ko'rinadi.";
  ...
  const criteriaHtml = criteria.length
    ? criteria
        .map((c) => {
          ...
          const pct2 =
            hasVal && c.maxScore > 0 ? Math.round((val / c.maxScore) * 100) : 0;
          const cc = !hasVal
            ? "var(--text3)"
            : pct2 >= 80
              ? "var(--teal-text)"
      ...
      <div style="height:10px;background:var(--bg4);border-radius:6px;overflow:hidden"><div style="height:100%;width:${pct2}%;background:${barBg};border-radius:6px;transition:width .6s ease"></div></div>
        ...
        .join("")
    : `<div style="text-align:center;padding:40px 20px;color:var(--text3)"><div style="font-size:40px;margin-bottom:10px">📋</div><div style="font-size:13px;font-weight:600">${noCriteriaYetLbl}</div></div>`;
...
function renderStudentTestsPage() {
  ...
  const { s, grp } = getCurrentStudentInfo();
  if (!s || !grp) {
    wrap.innerHTML = `<div style="text-align:center;padding:60px;color:var(--text3)"><div style="font-size:48px">📝</div><div style="font-size:15px;font-weight:600;margin-top:12px">Ma'lumot topilmadi</div></div>`;
    return;
  }
  ...
  const testsHtml = tests
    .map((t) => {
      ...
      const done = !!result;
      const pct = done
        ? Math.round((result.score / t.questions.length) * 100)
        : null;
      const pc = done
        ? pct >= 80
          ? "var(--teal-text)"
      ...
      const qLbl =
        LANG === "ru" ? "вопросов" : LANG === "en" ? "questions" : "ta savol";
      ...
      ${
        !done
          ? `onclick="startStudentTest(${t.id},${s.id})"
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px">
        ...
        ${
            ...
            ? `<div style="text-align:center;background:var(--bg2);border-radius:var(--r-md);padding:10px 16px;border:1px solid var(--border)">
          ...
          <div style="font-size:10px;color:var(--text3);">${result.score}/${t.questions.length}</div>
            ...
            : `<span style="background:var(--accent-light);color:var(--accent-text);padding:4px 10px;border-radius:20px;font-size:11px;font-weight:700;white-space:nowrap;border:1px solid var(--accent)">${newLbl}</span>`
        }
      </div>
...
function renderStudentChatPage() {
  const wrap = document.getElementById("student-chat-wrap");
  if (!wrap) return;
  const { s, grp } = getCurrentStudentInfo();
  if (!s || !grp) {
  ...
  const msgsHtml = msgs.length
    ? msgs
        .map((m) => {
          ...
          const time = new Date(m.ts).toLocaleTimeString("uz", {
            hour: "2-digit",
            minute: "2-digit",
          });
          const date = new Date(m.ts).toLocaleDateString("uz", {
            day: "2-digit",
            month: "short",
          });
      ...
      <div style="max-width:72%;padding:10px 14px;border-radius:${isMe ? "16px 4px 16px 16px" : "4px 16px 16px 16px"};background:${isMe ? "var(--accent)" : "var(--bg3)"};color:${isMe ? "#fff" : "var(--text)"};font-size:13px;line-height:1.5;box-shadow:var(--shadow-sm)">
        ...
        .join("")
    : `<div style="text-align:center;padding:40px;color:var(--text3)"><div style="font-size:48px;margin-bottom:12px">💬</div><div style="font-size:14px;font-weight:600">Hali xabar yo'q</div><div style="font-size:12px;margin-top:6px">Mentorga savol yuboring!</div></div>`;
  ...
  // Scroll to bottom
  setTimeout(() => {
    const m = document.getElementById("student-chat-page-msgs");
...
function showStudentDetailPopup(field) {
  ...
  if (field === "rating") {
    ...
    html = grp
      ...
      : '<div style="color:var(--text3)">Guruh topilmadi</div>';
  } else if (field === "debt") {
    title = "💸 To'lov holati";
...
function showMentorStatDetail(type) {
  ...
  if (type === "groups") {
    ...
    html = myGroups.length
      ? myGroups
          .map((g) => {
            const cnt = D.students.filter((s) => s.groupId === g.id).length;
            const active = D.students.filter(
              (s) => s.groupId === g.id && s.status === "Aktiv",
            ).length;
        ...
        <div style="margin-top:10px;display:flex;gap:6px">
          ...
          .join("")
      : '<div style="color:var(--text3);padding:16px 0;text-align:center">Guruhlar yo\'q</div>';
  } else if (type === "active") {
    title = "✅ Aktiv talabalar";
    html = activeStudents.length
      ...
      : '<div style="color:var(--text3);padding:16px 0;text-align:center">Aktiv talabalar yo\'q</div>';
  } else if (type === "all") {
    ...
    html = myStudents.length
      ...
      : '<div style="color:var(--text3);padding:16px 0;text-align:center">Talabalar yo\'q</div>';
    ...
    html = debtors.length
      ? debtors
        ...
        <span style="font-size:22px">💸</span>
          ...
          .join("")
      : '<div style="text-align:center;padding:30px;color:var(--teal-text)"><div style="font-size:36px">✅</div><div style="font-size:14px;font-weight:600;margin-top:8px">Qarzdor yo\'q!</div></div>';
  }
...
function calcAbsences(studentId, groupId) {
  ...
  Object.keys(D.attendance)
    .filter((k) => k.startsWith("att_" + groupId + "_"))
    .forEach((k) => {
...
function getVideos() {
  try {
    return JSON.parse(localStorage.getItem("edumanage_videos_v1") || "[]");
  } catch (e) {
    return [];
  }
}
...
function renderMentorVideos() {
  ...
  const emptyLbl =
    L === "ru"
      ...
      : L === "en"
        ...
        : "Hali darslik yo'q. Birinchisini qo'shing!";
  const countLbl =
    L === "ru" ? "Всего уроков" : L === "en" ? "Total lessons" : "Jami darslik";
  ...
  if (nc) {
    nc.textContent = vids.length;
    nc.style.display = vids.length ? "flex" : "none";
  }
}
...
function renderStudentVideos() {
  ...
  const emptyLbl =
    L === "ru"
      ? "Ментор ещё не добавил уроки"
      : L === "en"
        ? "Your mentor hasn't added lessons yet"
        : "Mentoringiz hali darslik qo'shmagan";
  ...
  wrap.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:20px">
      ${
        vids.length
          ? vids.map((v) => renderVideoCard(v, false)).join("")
          : `
    ...
    <div style="padding:14px">
      ...
      ${
        ...
        <button class="btn btn-del-outline" style="font-size:12px;padding:7px 12px" onclick="deleteVideo(${v.id})">🗑</button>
          ...
          : `<button class="btn btn-primary" style="width:100%;font-size:13px;padding:8px" onclick="${isYt ? `playYoutube('${v.url}','${v.id}')` : `playUploadedVideo(${v.id})`}">▶ ${watchLbl}</button>`
      }
    </div>
  </div>`;
}
...
function playYoutube(url, id) {
  ...
  overlay.style.cssText =
    "position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:9999;display:flex;align-items:center;justify-content:center";
  overlay.innerHTML = `
...
function playUploadedVideo(videoId) {
  ...
  if (!src) {
    toast("⚠️ Video topilmadi. Qayta yuklang.");
    return;
  }
  const ex = document.getElementById("video-player-overlay");
  if (ex) ex.remove();
...
function openVideoModal(editId) {
  ...
  const titleLbl = editId
    ? L === "ru"
      ? "Редактировать урок"
      : L === "en"
        ? "Edit Lesson"
        : "Darslikni tahrirlash"
    : L === "ru"
      ? "Новый видеоурок"
      : L === "en"
        ? "New Video Lesson"
  ...
  const cancelLbl =
    L === "ru" ? "Отмена" : L === "en" ? "Cancel" : "Bekor qilish";
  const ytLbl =
    L === "ru"
      ? "YouTube ссылка"
      : L === "en"
  ...
  modal.style.cssText =
    "position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:1000;display:flex;align-items:center;justify-content:center;padding:16px";
  modal.innerHTML = `
...
function saveVideo() {
  ...
  if (_editVideoId) {
    ...
    if (idx !== -1) {
      ...
      if (type === "youtube") {
        vids[idx].url = url;
        delete vids[idx].fileData;
        delete vids[idx].fileName;
      }
      if (type === "file" && _vFileData) {
        vids[idx].fileData = _vFileData;
        vids[idx].fileName = _vFileName;
        VIDEO_BLOB_CACHE[_editVideoId] = _vFileData;
      }
    }
  } else {
    const newV = {
    ...
    if (type === "file" && _vFileData) {
      newV.fileData = _vFileData;
      newV.fileName = _vFileName;
      VIDEO_BLOB_CACHE[newV.id] = _vFileData;
    }
    vids.push(newV);
  }
  ...
  const nc = document.getElementById("nc-mentor-videos");
  if (nc) {
    const cnt = getMentorVideosForUser().length;
    nc.textContent = cnt;
    nc.style.display = cnt ? "flex" : "none";
  }
}
...
function updateVideoNavLabels() {
  ...
  const lbl =
    L === "ru" ? "Видеоуроки" : L === "en" ? "Video Lessons" : "Darsliklar";
