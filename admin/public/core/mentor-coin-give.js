...
(function () {
  ...
  function cs() { return window.coinShop; }
        ...
        if (d && d.ok && d.data && d.data[COIN_KEY]) {
          try {
            ...
            if (typeof coins === 'object' && Object.keys(coins).length) {
              ...
              Storage.prototype.setItem.call(localStorage, COIN_KEY, d.data[COIN_KEY]);
            }
          } catch(e) {}
        }
        if (cb) cb();
  ...
  function toast(msg, color) {
    ...
    el.style.cssText = 'position:fixed;bottom:80px;left:50%;transform:translateX(-50%);'
      + 'background:var(--bg,#1e293b);border:2px solid ' + color + ';'
  ...
  window.csGiveCoinFromDetail = function (studentId, mentorName, groupId) {
    if (!cs()) { toast('⚠️ Coin tizimi yuklanmagan!', '#ef4444'); return; }
    ...
    if (amt <= 0) {
      ...
      if (inp) { inp.focus(); inp.style.borderColor = '#ef4444'; setTimeout(function () { inp.style.borderColor = ''; }, 1500); }
      return;
    }
    ...
    refreshCoinsFromBackend(function () {
      var mBal = gid ? cs().getMentorGroupBal(mentorName, gid) : cs().getMentorBal(mentorName);
      if (mBal <= 0) {
    ...
    var mBal2 = gid ? cs().getMentorGroupBal(mentorName, gid) : cs().getMentorBal(mentorName);
    if (false && mBal2 <= 0) {
  ...
  function _doGiveCoin(studentId, mentorName, gid, amt, mBal) {
    ...
    var newMBal = gid ? cs().getMentorGroupBal(mentorName, gid) : cs().getMentorBal(mentorName);
    ...
    toast('✅ ' + stuName + ' ga 🪙' + amt + ' berildi! Qoldi: 🪙' + newMBal, '#0d9488');
  ...
  function injectCoinUI(studentId) {
    ...
    var s = window.D && window.D.students && window.D.students.find(function (x) { return x.id === studentId; });
    if (!s) return;

    var cu = typeof getCurrentUser === 'function' ? getCurrentUser() : (window.getCurrentUser ? window.getCurrentUser() : {});
    var mentorName = cu.mentorName || cu.name || '';
    if (!mentorName || cu.role === 'Super Admin' || cu.role === 'Talaba') return;

    var grp = window.D && window.D.groups && window.D.groups.find(function (g) { return g.id === s.groupId; });
    var gid = s.groupId || (grp ? grp.id : null);

    // Avval localStorage dan o'qiymiz (tez)
    var mBal = cs() ? (gid ? cs().getMentorGroupBal(mentorName, gid) : cs().getMentorBal(mentorName)) : 0;
    var stuBal = cs() ? cs().getStudentBal(studentId) : 0;

    var coinBox = document.createElement('div');
    coinBox.id = 'mentor-coin-give-' + studentId;
    coinBox.style.cssText = 'background:linear-gradient(135deg,rgba(245,158,11,0.1),rgba(217,119,6,0.06));'
    ...
    function buildHTML(mB, sB) {
      ...
      return '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">'
        ...
        + '<button id="detail-coin-refresh-' + studentId + '" onclick="event.stopPropagation();_mcgRefresh(' + studentId + ',\'' + mEscName + '\',' + gid + ')" '
        ...
        + '<b id="detail-stu-bal-' + studentId + '" style="color:#d97706;font-size:14px">🪙 ' + sB + '</b></div>'
        + '<div style="font-size:12px;color:var(--text2,#94a3b8)">📚 Guruh balansi:<br>'
        + '<b id="detail-mentor-bal-' + studentId + '" style="color:#059669;font-size:14px">🪙 ' + mB + '</b></div>'
        + '</div>'
        ...
        + '<input id="detail-coin-inp-' + studentId + '" type="number" min="1" max="' + mB + '" placeholder="Miqdor"'
        ...
        + '</div>'
        + '<button onclick="event.stopPropagation();csGiveCoinFromDetail(' + studentId + ',\'' + mEscName + '\',' + gid + ')"'
    ...
    var actionDiv = detailBody.querySelector('[style*="display:flex;gap:8px;flex-wrap:wrap"]');
    if (actionDiv) {
    ...
    if (mBal <= 0) {
      refreshCoinsFromBackend(function () {
        var freshMBal = cs() ? (gid ? cs().getMentorGroupBal(mentorName, gid) : cs().getMentorBal(mentorName)) : 0;
